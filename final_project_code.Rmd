...
```{r}
# this is definitely hard-coded, however I couldn't get looping to go my way
# I'd rather have working code that makes sense instead of broken code
# indices are different in the paper than they are in R
...
measure. I will be taking a simpler approach, using the 
...
For the Table 2 simulation, I want to retain at least 283 observations.

```{r}
mean(c(N_vec[3], N_vec[4]))
```

For the Table 3 simulation, I want to retain at least 90 observations.

For simulation purposes, I may choose much larger amounts to retain. 
...
abc_sample = function(observed_data,
                      ...
                      epsilon, retain) {
  N = retain/epsilon
  samples = data.frame(matrix(ncol = 3, nrow = 0))
...
ggplot(post_plot_table_2) +
  ...
  ggtitle(label = "Figure 3(a)" , subtitle = "ABC posterior distributions for parameters 
          inferrred from data in Supplementary Table 2")
```
...
ggplot(post_plot_table_3) +
  ...
  geom_point(aes(qh2_post, qc2_post, colour = "blue")) +
  xlab("q_h") + ylab("q_c") + xlim(c(0, 1)) + ylim(c(0,1)) +
  ggtitle(label = "Figure 3(b)" , subtitle = "ABC posterior distributions for parameters 
          inferrred from data in Supplementary Table 3")
```


# Description

IT'S WORKINGGGG, actually for real, not a Star Wars joke anymore!!!!
